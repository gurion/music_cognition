...
def processScale(floating_point_time_series, sr):
    ...
    if bad_input(floating_point_time_series, sr):
        return 0
    ...
    error['pitch'], error['duration'], error['dynamics'] = normalize_pitch_error(rate_pitches(
        freqs)), normalize_duration_error(rate_durations(durations), sr), normalize_dynamics_error(rate_dynamics(dynamics))
...
def bad_input(input, sr):
    if (type(input) != type(np.ndarray([1, 2])) or type(sr) != type(2)):
        return True

    if (len(input) < 36000 or sr < 1):
        return True
